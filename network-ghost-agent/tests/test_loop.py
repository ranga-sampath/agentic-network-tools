...
def test_L1_loop_exits_on_complete_investigation(tmp_path):
    ...
    # adapter.generate called exactly once → loop exited after first turn
...
def test_L5_text_only_offers_continue_or_done(tmp_path):
    ...
    adapter = MagicMock()
    adapter.generate.side_effect = [text_resp, complete_resp]
    history = []
    ...
    # adapter.generate called once → user chose Done after the text-only response
    assert adapter.generate.call_count == 1
...
def test_L6_max_loop_turns_offers_extend_or_generate(tmp_path):
    ...
    mock_rca.assert_called_once()
    # adapter.generate not called because we hit the turn limit before calling the LLM
...
def test_L7_extend_increments_max_and_continues(tmp_path):
    ...
    adapter = MagicMock()
    adapter.generate.return_value = complete_resp
    history = []
    ...
    # After extending, the loop must call adapter.generate at least once
    assert adapter.generate.call_count >= 1
